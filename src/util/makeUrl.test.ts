...
describe('makeUrl', () => {
  ...
  it('makes replacements', () => {
    ...
    expect(url).toEqual(
      'https://localhost:8080/foz/baz?one=1&two=2#three=3&four=4',
    );
  });
  ...
  it('makes an absolute URL when given a base', () => {
    const url = makeUrl(
      ...
      'https://www.google.com/remove/this?q=hello',
    );

    expect(url).toEqual('https://www.google.com/search#three=3&four=4');
  });
});
